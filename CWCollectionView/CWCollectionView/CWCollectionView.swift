...
@objc protocol CWCollectionViewDelegate : UICollectionViewDataSource {
...
final class CWCollectionView : UICollectionView {
    @IBOutlet var cwDelegate: CWCollectionViewDelegate? {
        didSet {
            self.delegate = self
    ...
    override var delegate: UICollectionViewDelegate? {
        didSet {
            guard let delegate = delegate, delegate is UICollectionViewDelegateFlowLayout else { return }
...
extension CWCollectionView : UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        width = collectionView.bounds.size.width / (CGFloat(cwDelegate?.numberOfColumns() ?? 1))
